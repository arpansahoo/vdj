...
def nt_matches(
    stringa, stringb
):  # returns tuple: (% match, nt/total string, True/False VJ > 20)
    u = zip(
        stringa, stringb
    )  # This function might have terrible performance making it n^2, not sure though.
    y = []
...
class VDJ(object):
    def score_only_align(self, subtype):
        ...
        for ref in ref_seqs:
            alignments = pairwise2.align.localms(
                ref.seq, self.test_seq, 5, -10, -10, -10, score_only=True
            )
    ...
    def full_align(self, subtype):
        ...
        for ref in ref_seqs:
            alignments = pairwise2.align.localms(
                ref.seq, self.test_seq, 5, -10, -10, -10, score_only=False
            )
        ...
        ] = tmp["%sALIGNMENT" % subtype].apply(pd.Series)
        test_hit = lambda x: x["%sTESTSEQ" % subtype][
            x["%sALIGNSTART" % subtype] : x["%sALIGNSTOP" % subtype]
        ]
        ref_hit = lambda x: x["%sREFSEQ" % subtype][
            x["%sALIGNSTART" % subtype] : x["%sALIGNSTOP" % subtype]
        ]
        remainder_hit = lambda x: (
            x["%sTESTSEQ" % subtype][: x["%sALIGNSTOP" % subtype]]
        ) + (x["%sREFSEQ" % subtype][x["%sALIGNSTOP" % subtype] :])
        ...
        tmp = tmp.iloc[0]
        (
            tmp["%s Match Percent" % subtype],
            tmp["%s Match" % subtype],
            tmp["%s Match Length" % subtype],
        ) = nt_matches(tmp["%sREFALIGNSEQ" % subtype], tmp["%sTESTALIGNSEQ" % subtype])
        tmp = tmp.drop(
            labels=["{subtype}ALIGNMENT".format(subtype=subtype)]
        )  # try dropping this, seems to be causing crashes
    ...
    def score_test(self):
        ...
        if jdf["JSCORE"] <= self.jthreshold:
            jdf["JID"] = "No Match"
            self.results = jdf
            self.results["JUNCTION"], self.results["CDR3"], self.results["REASON"] = (
                "No Match",
                "No Match",
                "No Match",
            )
    ...
    def junction_find(self):
        junctions = re.search(
            r"((tgt|tgc)(...){4,20}(ttt|ttc|tgg)(ggt|ggc|gga|ggg)(...)(ggt|ggc|gga|ggg))",
            self.results["JREMAINDERSEQ"],
        )
        if junctions is None:
            ...
            if "*" not in translation and (
                ...
                (
                    ...
                    self.results["CDR3"],
                    self.results["REASON"],
                ) = (junction, translation, "NA")
            else:
                (
                    self.results["JUNCTION"],
                    self.results["CDR3"],
                    self.results["REASON"],
                ) = ("Unproductive", "Unproductive", "Stop")
