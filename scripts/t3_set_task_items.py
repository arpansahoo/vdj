...
dir = '/work/pi_gblanck/Arpan/NBL_WXS/NBL_BAMS_Results/' #str(sys.argv[1])
max_size = 40000.0
...
def findfiles_and_make_df(path, receptor):
    ...
    print("Found {nfiles} {receptor} files in {path}".format(nfiles=len(files), receptor=receptor, path=path))
...
with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:
    ...
    for receptor in receptor_list:
        ...
        futures.append(executor.submit(findfiles_and_make_df, path=path, receptor=receptor))
