...
class VDJrecord(object):
    ...
    def __init__(self, cancer="TCGA-COAD", samples_path = None, vdjdb_path = None, clinical_path = None):
        self.cancer = cancer
    ...
    def load_raw_csv_path(self,path):
        ...
        self.raw = pd.DataFrame()
        for file in os.listdir(path):
            if file.endswith(".csv") and not file.startswith('~') and not file.startswith('V'):
                filepath = os.path.join(path, file)
                print("Loading: %s"%filepath)
    ...
    def threading(self, threading_db_path):
        ...
        def perform_threading(df, threading_db_path):
            for side in ["V", "J"]:
                df[["%s_Accession"%side, "%sID"%side, "drop"]] = df["%sID"%side].str.split("|", expand=True)
            ...
            df["V_CDR3_J"] = df.apply(lambda x: x['CDR3'][(x['CDR3'].find(x['V_CDR3']) + len(x['V_CDR3'])):], axis=1)

            df["J_CDR3"] = df.apply(lambda x: compute_jcdr3(x["V_CDR3_J"], x["J.AA.String"]), axis=1)
            df = df[df['J_CDR3'].notna()]

            df["V_CDR3_J"] = df.apply(lambda x: chop_v(x["V.AA.String"], x["V_CDR3"]), axis=1)                
            df["V_CDR3_J"] += df["CDR3"]
            df["V_CDR3_J"] += df.apply(lambda x: chop_j(x["J.AA.String"]), axis=1)

            return df

        self.filtered_productive = perform_threading(self.filtered_productive, threading_db_path)
